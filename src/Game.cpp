...
SDL_Surface *aggressiveSurfActive, 
            *scoutSurfActive, 
            *standGroundSurfActive, 
            *defensiveSurfActive,
            *deploySurfPressed, 
            *destructSurfPressed;

SDL_Surface *gameMenuSurf, 
            *resumeGameSurf, 
            *saveGameSurf, 
            *loadGameSurf,
	    *gameSettingsSurf, 
...
SDL_Surface *resumeGameSurfPressed, 
            *saveGameSurfPressed, 
            *loadGameSurfPressed,
	    *gameSettingsSurfPressed, 
            *quitSurfPressed, 
            *restartSurfPressed;
...
Button	unitButton_Aggressive,
        ...
        settingsOkButton,
        settingsGameSpeedPlus,
        settingsGameSpeedMinus,
        settingsGameSpeedBar,
        settingsVolumePlus,
        settingsVolumeMinus,
        settingsVolumeBar;
...
void game_setSaveGameSpot(void* data)
{
	...
	if ((spot >= 0) && (spot < NUM_SAVEGAMES))
	{
		...
		gameSaveNameEntry.handlePress(0, 0);
...
void setAttackMode(void* data)
{
	if (!selectedList->isEmpty())
	{
		ATTACKTYPE newAttackMode = (ATTACKTYPE)((long)data);
	...
	if (!fullscreenMode)
	{
		...
		if (!SDL_MUSTLOCK(screen) || (SDL_LockSurface(screen) == 0))
		{
			...
			if (thisPlayer->getPowerRequirement() == 0)
			{
				...
				yCount2 = int(round((double)thisPlayer->getPower()/(double)thisPlayer->getPowerRequirement()*(double)powerIndicatorPos.h/2));
			...
			if (thisPlayer->getCapacity() == 0)
				...
				yCount2 = int(round((double)thisPlayer->getStoredCredits()/(double)thisPlayer->getCapacity()*(double)spiceIndicatorPos.h));
...
//DRAW MESSAGEBOX
...

	{
		...
		sprintf(temp, "fps:%d, frameTimer:%d", int(round(fps)), frameTimer);

...
//    	    LOG_INFO("UnitClass", "move %d-%d", m_xSpeed, m_ySpeed);
		}
		else
		{
...
void UnitClass::setSpeeds()
{
	...
	switch (m_drawnAngle)
	{
	case (LEFT): m_xSpeed = -m_speed; m_ySpeed = 0; break;
	case (LEFTUP): m_xSpeed = -m_speed*DIAGONALSPEEDCONST; m_ySpeed = m_xSpeed; break;
	case (UP): m_xSpeed = 0; m_ySpeed = -m_speed; break;
	case (RIGHTUP): m_xSpeed = m_speed*DIAGONALSPEEDCONST; m_ySpeed = -m_xSpeed; break;
	case (RIGHT): m_xSpeed = m_speed; m_ySpeed = 0; break;
	case (RIGHTDOWN): m_xSpeed = m_speed*DIAGONALSPEEDCONST; m_ySpeed = m_xSpeed; break;
	case (DOWN): m_xSpeed = 0; m_ySpeed = m_speed; break;
	case (LEFTDOWN): m_xSpeed = -m_speed*DIAGONALSPEEDCONST; m_ySpeed = -m_xSpeed;
	}

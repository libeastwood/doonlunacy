...
/*	showMessage("using builderclass");
	...
	if (selectionList->getNumElements() >= 1)
	{
		...
		sprintf(temp, "%s: %.2f", item->getName(), round(priceModifier * item->getCost()));
		b1.setHelpText(temp);
...
void BuilderClass::loadBuildList(FILE* fp)
{
	...
	for (i = 0; i < numItems; i++)
	{
		...
		if (found)
		{
			...
			buildItem->setCost(int(round(tempDouble)));
			fread(&tempDouble, sizeof(double), 1, fp);
			buildItem->setProgress(int(round(tempDouble)));
			fread(&temp, sizeof(int), 1, fp);
...

			{
...

				{
					...
					if (destination.x != NONE)
					{
						...
						newUnit->setAngle(int(round(8.0/256.0*dest_angle(newUnit->getLocation(), newUnit->getDestination()))));
					}

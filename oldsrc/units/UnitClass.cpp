...
void UnitClass::blitToScreen()
{
	...
	if (!destroyed)
	{
		...
		if (badlyDamaged)
			...
			drawSmoke(int(round(realX - dborder->minX)), int(round(realY - dborder->minY)));
		if (selected)
...

{
...

	{
		...
		targetAngle = int(round(8.0/256.0*dest_angle(&location, &targetLocation)));
...

{
...

	{
...

		{
                        ...
                        COORDTYPE cp = object->getClosestPoint(&location);
			return (/*objectDistance*/blockDistance(&location, &cp) <= weaponRange);
...

	{
...

		{
...

			{
...

				{
					...
					tempRepairYard = ((RepairYardClass*)tempStructure);
                                        ...
                                        COORDTYPE cp = tempRepairYard->getClosestPoint(&location);
					tempDistance = distance_from(&location, &cp);
...

			{
				...
				if (settings.playerType != CLIENT)
                                {
                                        COORDTYPE cp = bestRepairYard->getClosestPoint(&location);
					netDoCommand(bestRepairYard, &cp, true);
                                }
			}
...

	{
...

		{
...

			{
				...
				if (closestStructure)
                                {
                                        COORDTYPE cp = closestStructure->getClosestPoint(&location);
					netDoCommand(NULL, &cp, false);
                                }
				else

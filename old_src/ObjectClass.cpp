...

{
	...
	centrePoint->x = int(round(realX));

	centrePoint->y = int(round(realY));
...

{
	...
	if (!structureList->isEmpty())
	{
		...
		while(structureList->currentNotNull())
		{
			...
			{
                                COORDTYPE cp = tempStructure->getClosestPoint(object->getLocation());
				structureDistance = blockDistance(object->getLocation(), &cp);
...
ObjectClass* ObjectClass::findClosestTargetUnit(ObjectClass* object)
{
	...
	if (!unitList->isEmpty())
	{
		...
		while(unitList->currentNotNull())
		{
			...
			{
                                COORDTYPE cp = tempUnit->getClosestPoint(object->getLocation());
				unitDistance = blockDistance(object->getLocation(), &cp);
